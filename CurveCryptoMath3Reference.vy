...
def _exp(_power: int256) -> uint256:
    ...
    """
        ...
        所以: 
            e^x = 2^x•㏒₂(e)
        因为:
            ㏒₂(e) 和 ㏒_e(2) 互为倒数， 而  ㏒_e(2) 的另一个写法是 ㏑(2)
        所以:
            ㏒₂(e) 和 1/㏑(2) 相等
    ...
    """
        约 -42.139, 这是因为在 1e18 的精度下, e^-42.13 得到的小数已经无穷小，低于 1e18 的最小精度，可以直接视作 0。
    """
    if _power <= -42139678854452767551:
        return 0

    """
        约等于 135.305,  e^135.305•10^18 结果逼近 2^255 , 这会突破 EVM 中 uint256 类型的最大存储极限
    """
    if _power >= 135305999368893231589:
        raise "exp overflow"

    # 转化为二进制的 96 位定点数（Q-point format)
    x: int256 = unsafe_div(unsafe_mul(_power, 2**96), 10**18)

    """
        54916777467707473351141471128 就是 ㏑(2)•2^96
        2^95 是 2^96 的一半

        k 的本质就是：当我们把底数从 e 强行切换为更容易被计算机理解的底数 2 之后，所得到的“最接近的整数次幂”

        这里的 k 就是"四舍五入"之后的指数部分
    """
    k: int256 = unsafe_div(
        unsafe_add(
            unsafe_div(unsafe_mul(x, 2**96), 54916777467707473351141471128),
            """
                这里加 2^95 是为了在整数除法中实现"四舍五入"

                例如: 2.3 + 0.5 = 2.8, 向下取整是 2。
                例如: 2.6 + 0.5 = 3.1, 向下取整是 3。
            """
            2**95,  
        ),
        2**96,
    ...
    """
        k•㏑(2) 这就相当于算出了“能够用底层位移直接解决的那部分大小”

        e^r: 计算机不能直接算，必须用多项式去模拟。

        这个范围的推导，完完全全是由我们在算 k 时, 加了 2^95 (即 +0.5)实现四舍五入的那一步决定的。

        余数 r 一定在 [-㏑(2)/2 , ㏑(2)/2] 范围内， 就是 [0.5•-㏑(2) , 0.5•㏑(2)]

        不管一个小数是多少，当我们把它四舍五入到最接近的整数时，这个小数和最终整数之间的“误差”（距离），
        绝对不可能超过 0.5。
        举例: 3.4 离 3 的距离是 0.4
        举例: 3.9 离 4 的距离是 -0.1
